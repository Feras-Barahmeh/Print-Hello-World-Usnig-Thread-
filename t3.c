...
pthread_mutex_t  mut;
pthread_cond_t 	 producer_cv; 	// conditional variable
...
void *producer(void *arg)
{
  ...
  while (!producer_done) {
  		...
  		if (supply < NUM_ITEMS_PER_PRODUCE) 
  			supply ++;
  		if (supply == NUM_ITEMS_PER_PRODUCE && supply != 0)
				pthread_cond_wait(&producer_cv, &mut);
		if (supply == NUM_ITEMS_PER_PRODUCE) 
			producer_done++; // To end loop 
...
void *consumer(void *arg)
{
  ...
  pthread_mutex_lock(&mut);
  while (supply == 0) //  inventory remaining 
